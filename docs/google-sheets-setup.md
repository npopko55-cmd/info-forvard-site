> ⚠️ **УСТАРЕЛО.** Эта схема (Google Apps Script + Таблица) отправляла перс. данные в Google (США) —
> нарушение 152-ФЗ. После переезда на reg.ru (тариф Host-Lite без PHP) форма переведена на
> **Яндекс Формы** (РФ): встроена в модальное окно на странице контактов, данные хранятся в Яндексе,
> уведомления идут на почту. ID формы задан в `src/components/contact-form.tsx`. Файл оставлен для истории.
